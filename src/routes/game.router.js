...
router.post('/games/play/:characterId', authMiddleware, async (req, res, next) => {
  ...
  const teamACharacter = prisma.character.findFirst({
    //A팀 캐릭터 정보
    where: {
      UserId: userId,
    },
  ...
  const teamBCharacter = prisma.character.findFirst({
    //B팀 캐릭터 정보
    where: {
      characterId,
    },
  ...
  const teamARoster = prisma.roster.findFirst({
    //A팀 출전 명단
    where: {
  ...
  const teamBRoster = prisma.roster.findFirst({
    //B팀 출전 명단
    where: {
  ...
  const teamAPlayerList = teamARosterList.map((element) => {
    //A팀 로스터 선수 중 보유 선수 정보
    return prisma.characterPlayer.findFirst({
      //A팀 조회
      where: {
  ...
  const teamBPlayerList = teamBRosterList.map((element) => {
    //B팀 로스터 선수 중 보유 선수 정보
    return prisma.characterPlayer.findFirst({
      //A팀 조회
      where: {
        characterPlayerId: element,
  ...
  const teamAPlayerInfo = teamAPlayerList.map((element) => {
    //A팀 로스터 선수 스텟 조회
  ...
  const teamBPlayerInfo = teamBPlayerList.map((element) => {
    //B팀 로스터 선수 스텟 조회
    return prisma.character.findFirst({
      where: {
        playerId: element.playerId,
