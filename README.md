...
회원가입 후 로그인을 한다  
기본적으로 10000캐시가 지급이 되기 때문에 이 캐시로 선수 뽑기를 진행한다  
...
선수가 마음에 들지 않는다면 방출을 할 수도 있고 이적이나 트레이딩을 할 수 있다  
선수 방출을 하게 되면 방출 패널티로 뽑기에 쓰이는 캐시가 증가한다  
...
- 도전 구현 사항
  ...
  - 선수 강화
    ...
    - 강화 확률 선정 기준 - 0강 + 0강 = 100% - 1강 + 1강 = 80% - 2강 + 2강 = 60% - 3강 + 3강 = 40% - 4강 + 4강 = 20% - 강화 대상의 강화 레벨 = 강화 재료의 강화 레벨
            -> 고유 확률 (ex. 0강 + 0강 = 100%, 3강 + 3강 = 40%)
          - 강화 대상의 강화 레벨 < 강화 재료의 강화 레벨 -> 100%
